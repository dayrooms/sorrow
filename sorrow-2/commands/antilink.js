    ...
    if (talkedRecently.has(message.author.id)) {
      ...
      let nukeable = new EmbedBuilder()
       ...
       if (args[0] === "off") {
        ...
        if ((await await db.has(`antilink_${message.guild.id}`)) === true) {
          await await db.delete(`antilink_${message.guild.id}`);
          message.reply({ embeds: [disabled] }).catch(() => {
            /*Ignore error*/
          });
        }
      } else if (args[0] === "on") {
        if ((await await db.has(`antilink_${message.guild.id}`)) === false) {
          await await db.set(`antilink_${message.guild.id}`, true);
        ...
        let embed11 = new EmbedBuilder()
          ...
          .setDescription(
            `🛡️  Sorrow anti link \n • antilink on - toggles antilink event on \n • antilink off - toggles antilink event off \n • antilink whitelist \n • antilink blacklist \n • antilink whitelisted`
          )
      ...
      if (!args[0]) {
        ...
        let checkdisabled = new EmbedBuilder()
          ...
          .setColor(color);
        let antinuke = await db.get(`antilink_${message.guild.id}`);
        if (antinuke !== true) {
        ...
        await db.push(`linktrusted_${message.guild.id}`, data);
        ...
        let database = await db.get(`linktrusted_${message.guild.id}`);
        if (database) {
          ...
          await db.set(`linktrusted_${message.guild.id}`, filter);
