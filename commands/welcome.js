...
module.exports = {
  ...
  guildOnly: false,
    ...
    const embed = new EmbedBuilder()
      .setDescription(`\👋  **greet setup** \n • greet channel \n • greet embed • greet message \n • greet footer \n • greet author \n • greet image \n • greet removeimage \n • greet clear \n • greet stats \n • greet variables \n • greet color`)
      .setColor(color)
    ...
    if (args[0] === 'message') {
      await db.set(`welmessage_${message.guild.id}`, args.splice(1).join(' '))
      let wlcmsg = await db.get(`welmessage_${message.guild.id}`)
      if (wlcmsg === null) {
      ...
      let welcome = await db.get(`welmessage_${message.guild.id}`);
      if (welcome === null) {
                const setmsgembed = new EmbedBuilder()
          .setDescription(`${xmark} There is no welcome message set one with ${prefix}welcome message`)
          .setColor(error)
      ...
      let footer = await db.get(`welcembed_${message.guild.id}`);
      if (footer === null) { footer = `` }
      let image = await db.get(`image_${message.guild.id}`)
      if (image === null) {
        //image = `https://cdn.discordapp.com/attachments/989999587890712606/990905254138765362/Screenshot_7.png`
      } 
      let author = await db.get(`author_${message.guild.id}`)
      if (author === null) {
        author = ""
      }
       let colors = await db.get(`color_${message.guild.id}`) 
       if(colors === null) {
         colors = color;
       }
      let thumbnail = await db.get(`thumbnail_${message.guild.id}`)
      if(thumbnail === null) {
        thumbnail = ` `
      }
      if(thumbnail === '')
     ...
     // .setThumbnail(`${thumbnail}`)
      ...
      await db.delete(`welcembed_${message.guild.id}`);
      await db.delete(`welmessage_${message.guild.id}`)
      await db.delete(`image_${message.guild.id}`)
      await db.delete(`author_${message.guild.id}`)
      await db.delete(`color_${message.guild.id}`)
      await db.delete(`thumbnail_${message.guild.id}`)
      return await message.reply({embeds:[deleted]})
    }
    else if(args[0] === "footer"){
      await db.set(`welcembed_${message.guild.id}`, args.splice(1).join(' '))
        let footers = await db.get(`welcembed_${message.guild.id}`);
      ...
      return await message.reply({embeds:[footemebed]})
        ...
        .setColor(color)
         await db.set(`image_${message.guild.id}`,args[1])
      ...
      icon = message.attachments.first().url
         console.log(icon)
      await db.set(`image_${message.guild.id}`, icon || args[1])
        return message.reply({embeds:[x]})
      .catch(() => {
    ...
    image = image.replace('{user.icon}',message.author.displayAvatarURL({dynamic:true,size:4096}))
      ...
      .setDescription(`👋 ${message.guild.name} Welcome Stats `)
      .setColor(color)
            .addFields({
    ...
    },
    {
      ...
      .setColor(color)
      await db.set(`author_${message.guild.id}`, args.splice(1).join(' '))
      ...
      .setColor(color)
      await db.set(`color_${message.guild.id}`, args.splice(1).join(' '))
      return message.reply({embeds:[authorembed]})
        ...
        .setColor(color)
         await db.set(`thumbnail_${message.guild.id}`,args[1])
        message.reply({embeds:[x]})
      ...
      icon = message.attachments.first().url
         console.log(icon)
      await db.set(`thumbnail_${message.guild.id}`, icon || args[1])
        return message.reply({embeds:[x]})
      .catch(() => {
        message.reply({embeds:[no]})
      ...
      if(args[1] == 'on'){
        await db.set(`embedoff_${message.guild.id}`,true)
        message.reply({embeds:[{description:`${checked} Succesfully enabled embed`,color:color}]})
      }else if(args[1] == 'off'){
        await db.delete(`embedoff_${message.guild.id}`)
