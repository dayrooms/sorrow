    ...
    let emoji = `• `;
      ...
      try {
       ...
       // if (!mentionedMember.permissions.has([ Permissions.FLAGS.ADMINISTRATOR]))  perms = `${`${mentionedMember.permissions.toArray().sort((a, b) => a.localeCompare(b)).map(p=> `\`${p}\``).join(", ")}`}`
        ...
        .setTitle(`${mentionedMember.user.username} ${mentionedMember.presence.status.replace('dnd','⛔').replace('idle','🌙').replace('online','🟢 ').replace('offline','⚫')} \`${mentionedMember.user.id}\``)
        ...
        {
            name:`Created`,
           // value: `🕒  ${moment(mentionedMember.createdAt).format("dddd MMMM YYYY")}`,
